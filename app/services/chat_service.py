...
GROUP_CONTEXT = {
    ...
    "FOUNDER": "ОСНОВАТЕЛЬ Павлов Владимир Геннадьевич. Стиль: уважительный, оперативный, инициативный. Исполнять все поручения. Докладывать о результатах. ",
}
...
class ChatService:
    ...
    async def _process_founder(self, db, client_id, channel, message, history, start_time, session_id):
        ...
        if not history:
            welcome = "Здравствуйте, Владимир Геннадьевич! Я готов к работе. Что поручите?"
            await self.dialog.save_message(db, session_id, client_id, channel, "user", message)
